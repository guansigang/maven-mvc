		...
		if (selected != null) {
			$.messager.confirm('提示', '是否确定要删除？', function(y) {
				if (y) {
					...
					$.post("userCenter/deleteSysBaseDictItem", {
						item_code : item_code
					}, function(data) {
...
<body>
	<div class="easyui-layout" style="width:100%;">
		<div>
			<form id="detailff" method="post"
				action="${ contextPath }logistics/querySysBaseShiro">
				<table class="easyui-datagrid" id="mainShiroInfoTable" title=""></table>
				<div id="detailtb" style="padding:5px;height:auto">
					<div>
						模块查询: <input class="easyui-combobox" id="security_id"
							name="security_id" style="width:110px"></input> 路径查询: <input
							class="easyui-textbox" id="shiro_path" name="shiro_path"
							style="width:110px"></input> <a href="javascript:void(0)"
							title="清空搜索项" class="easyui-linkbutton" iconCls="icon-remove"
							onclick="clearDetailForm()">清空</a> <a href="javascript:void(0)"
							title="清空搜索项" class="easyui-linkbutton" iconCls="icon-search"
							onclick="submitDetailForm()">搜索</a> <a href="javascript:void(0)"
							title="添加模块" class="easyui-linkbutton" iconCls="icon-add"
							onclick="add_shiro();return false;">添加</a> <a
							href="javascript:void(0)" title="修改模块" class="easyui-linkbutton"
							iconCls="icon-edit" onclick="edit_shiro();return false;">修改</a> <a
							href="javascript:void(0)" title="删除模块" class="easyui-linkbutton"
							iconCls="icon-cancel" onclick="deleteShiro();return false;">删除</a>
					</div>
				</div>
			</form>
		</div>
		<div id="editShiroWin">
			<form id="editShiroForm" style="margin-top: 50px;" method="post">
				<table style="padding: 20px;margin: 0 auto;">
					<input type="hidden" id="shiro_id_fix" name="shiro_id"
						class="easyui-textbox" />
					<tr>
						<td>所属权限:</td>
						<td><input type="text" id="security_id_fix"
							name="security_id" class="easyui-textbox" /></td>
						<td>
					</tr>
					<tr>
						<td>资源路径:</td>
						<td><input type="text" id="shiro_path_fix" name="shiro_path"
					...
					<tr>
						...
						<td colspan=4><input class="easyui-textbox"
							id="path_desc_fix" name="path_desc" data-options="multiline:true"
							value="" style="width:100%;height:50px"></td>
						<td>
					</tr>
				</table>
			</form>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#mainShiroInfoTable').datagrid({
				...
				columns : [ [ {field : 'shiro_id',checkbox : true},
		...
		function submitDetailForm() {
			...
			var shiro_path = $("#shiro_path").val();
			$('#mainShiroInfoTable').datagrid('load', {
				security_id : security_id,
				shiro_path : shiro_path
			});
		}

		function clearDetailForm() {
			$("#security_id").combobox('setValue', '');
			$("#shiro_path").textbox('setValue', '');
		}
			...
			if (selected != null) {
				$.messager.confirm('提示', '是否确定要删除？', function(y) {
					if (y) {
						...
						$.post("userCenter/deleteSysBaseShiro", {shiro_id : shiro_id},
							 function(data) {
